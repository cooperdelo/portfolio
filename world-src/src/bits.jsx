...
// A framed photo tile. Click opens the FULL info lightbox (title + meta table +
// blurb, basement-style) when `info` is provided — the readable version of the
// tiny hover card. Hover shows a compact preview card.
export function InfoTile({ src, position, rotation = [0, 0, 0], width = 1.4, accent = '#F4EFE6', card, cardSide = 1, caption, info }) {
  const g = useRef();
  const rim = useRef();
  const [hover, setHover] = useState(false);
  const openLightbox = useStore((s) => s.openLightbox);
  ...
  const open = () => {
    if (info) openLightbox({ src: P + src, accent, eyebrow: info.eyebrow, title: info.title, meta: info.meta || [], blurb: info.blurb });
    else openLightbox(P + src, caption || prettyCaption(src));
  };
  return (
    <group ref={g} position={position} rotation={rotation}
      ...
      <Image url={thumb(src)} transparent position={[0, 0, 0.031]} scale={[width, h]} toneMapped={false} />
      {hover && card && (
        <Html position={[cardSide * (width / 2 + 0.15), 0.05, 0.12]} transform occlude zIndexRange={[25, 0]} distanceFactor={3.0} style={{ pointerEvents: 'none' }}>
          <div style={{
            width: 210, padding: '12px 14px', borderRadius: 11,
            ...
            <div style={{ fontFamily: 'Geist Mono, monospace', fontSize: 8.5, letterSpacing: '.14em', textTransform: 'uppercase', color: accent, marginTop: 8, opacity: 0.9 }}>Click to read full card →</div>
          </div>
        </Html>
      )}
    </group>
...
export function Panel({ position, rotation = [0, 0, 0], width = 260, children, distanceFactor = 3.0, accent = "#F4EFE6" }) {
  return (
    <Html position={position} rotation={rotation} transform occlude zIndexRange={[25, 0]} distanceFactor={distanceFactor} style={{ pointerEvents: 'none' }}>
      <div style={{
...
export function Embed({ position, rotation = [0, 0, 0], url, kind = 'spotify', label, accent = '#F4EFE6', w = 340, h = 460, df = 2.3 }) {
  return (
    <group position={position} rotation={rotation}>
      <Html transform occlude zIndexRange={[25, 0]} distanceFactor={df} style={{ pointerEvents: 'auto' }}>
...
export function ContactStation({ position, rotation = [0, 0, 0], accent = '#F4EFE6' }) {
  return (
    <Html position={position} rotation={rotation} transform occlude zIndexRange={[25, 0]} distanceFactor={2.2}>
      <div style={{
